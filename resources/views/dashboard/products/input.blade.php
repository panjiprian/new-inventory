        ...
        <form action="/input-barang" method="POST" enctype="multipart/form-data" class="w-1/2 mt-5">
            ...
            <div class="mt-3">
                ...
                <div class="border-2 p-1 @error('unique_code') border-red-400 @enderror">
                    <input name="unique_code" id="unique_code" value="{{ old('unique_code') }}" class="text-black w-full h-full focus:outline-none text-sm" type="text" readonly>
                </div>
                @error('unique_code')
            ...
            <div class="mt-3">
                ...
                <div class="border-2 p-1 @error('description') border-red-400 @enderror">
                    <textarea name="description" id="description" class="text-black w-full h-full focus:outline-none text-sm" rows="4">{{ old('description') }}</textarea>
                </div>
                @error('description')
                    <p class="italic text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
                ...
                if (categoryId) {
                    $.ajax({
                        ...
                        success: function (response) {
                            $('#variant').html('<option value="" disabled selected>Select Variant</option>'); // Reset variant
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.unique_code) {
                        $('#unique_code').val(response.unique_code); // ✅ ID sudah sesuai dengan form
                    }
                },
                error: function(xhr) {
                    console.log(xhr.responseText);
