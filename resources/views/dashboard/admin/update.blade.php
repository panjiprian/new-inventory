...
<div class="container px-4">
    <div class="bg-white p-5 mt-5 rounded-lg">
        <div class="flex">
            <h2 class="text-gray-600 font-bold">Update Admin</h2>
        </div>
        ...
        <form action="/ubah-admin/{{$admin->id}}" method="POST" class="w-1/2 mt-5">
            ...
            <div class="mt-3">
                ...
                <div class="@error('email') border-red-400 @enderror border-2 p-1">
                    <input autocomplete="off" type="email" value="{{$admin->email}}" name="email" class="text-sm w-full h-full focus:outline-none" id="email">
                </div>
                @error('email')
                    <p class="italic text-red-500 text-sm mt-1">{{$message}}</p>
                @enderror
            ...
            <div class="mt-3">
                ...
                <div class="@error('phone') border-red-400 @enderror border-2 p-1">
                    <input autocomplete="off" type="tel" name="phone" id="phone" class="text-sm w-full h-full focus:outline-none" value="{{$admin->phone ?? '+62'}}">
                </div>
                @error('phone')
                    <p class="italic text-red-500 text-sm mt-1">{{$message}}</p>
                @enderror
